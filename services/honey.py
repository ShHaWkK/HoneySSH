...
# ================================
# Fonctions dynamiques pour simuler les fichiers système
# ================================
def get_dynamic_df():
...
def get_dynamic_meminfo():
    total_mem = random.randint(4000000, 8000000)  # Total memory in kB (4-8 GB)
...
def get_dynamic_messages():
    ...
    for i in range(10):
        ...
        service = random.choice(["sshd", "systemd", "cron", "nginx", "apache2"])
...
def get_dev_null():
    return ""  # /dev/null retourne toujours une chaîne vide

def get_dev_zero():
    return "\0" * 1024  # Simule 1 Ko de zéros
...
BASE_FILE_SYSTEM = {
    "/": {"type": "dir", "contents": ["bin", "sbin", "usr", "var", "opt", "root", "home", "etc", "tmp", "secret", "proc", "dev", "sys"]},
    "/bin": {"type": "dir", "contents": ["bash", "ls", "cat", "grep", "chmod", "chown", "mv", "cp"]},
    "/sbin": {"type": "dir", "contents": ["init", "sshd", "iptables", "reboot"]},
    "/usr": {"type": "dir", "contents": ["bin", "lib", "share", "local"]},
    "/usr/bin": {"type": "dir", "contents": ["python", "gcc", "make", "apt-get", "vim", "nano"]},
    "/usr/sbin": {"type": "dir", "contents": ["apache2", "postfix", "nginx"]},
    "/usr/local": {"type": "dir", "contents": ["bin"]},
    "/usr/local/bin": {"type": "dir", "contents": ["custom_script.sh"]},
    "/var": {"type": "dir", "contents": ["log", "mail", "www", "run"]},
    "/var/log": {"type": "dir", "contents": ["syslog", "auth.log", "messages", "dmesg"]},
    "/var/www": {"type": "dir", "contents": ["index.html", "index.php"]},
    "/var/run": {"type": "dir", "contents": ["sshd.pid", "nginx.pid"]},
    ...
    "/etc": {"type": "dir", "contents": ["passwd", "shadow", "apt", "service", "hosts", "myconfig.conf", "fstab", "resolv.conf", "hosts.allow", "ssh", "nginx", "apache2"]},
...
BASE_FILE_SYSTEM["/tmp/tmpfile1.txt"] = {"type": "file", "content": "Temporary file created by system process\n"}
BASE_FILE_SYSTEM["/tmp/tmpsocket.sock"] = {"type": "file", "content": ""}
BASE_FILE_SYSTEM["/etc/fstab"] = {
    "type": "file",
    "content": f"""# /etc/fstab: static file system information.
/dev/sda1  /  ext4  defaults,errors=remount-ro  0  1
tmpfs  /tmp  tmpfs  nosuid,nodev  0  0
/dev/sdb1  /home  ext4  defaults  0  2
swap  /dev/sda2  swap  sw  0  0
"""
}
BASE_FILE_SYSTEM["/etc/resolv.conf"] = {
    "type": "file",
    "content": """nameserver 8.8.8.8
nameserver 8.8.4.4
"""
}
BASE_FILE_SYSTEM["/etc/hosts.allow"] = {
    "type": "file",
    "content": """sshd: 192.168.1.0/24
all: localhost
"""
}
BASE_FILE_SYSTEM["/etc/ssh/sshd_config"] = {
    "type": "file",
    "content": f"""# OpenSSH configuration file
Port 22
Protocol 2
PermitRootLogin no
...
BASE_FILE_SYSTEM["/etc/nginx/nginx.conf"] = {
    "type": "file",
    "content": f"""user www-data;
...
BASE_FILE_SYSTEM["/etc/apache2/apache2.conf"] = {
    "type": "file",
    "content": f"""ServerRoot "/etc/apache2"
Listen 80
...
BASE_FILE_SYSTEM["/var/www/index.php"] = {
    "type": "file",
    "content": "<?php\n echo 'Welcome to the server website!';\n?>\n"
}
BASE_FILE_SYSTEM["/usr/local/bin/custom_script.sh"] = {
    "type": "file",
...
# ================================
# Alerte avec logs détaillés
# ================================
def trigger_alert(session_id, event_type, details, client_ip, username):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"[ALERTE] {timestamp} - {client_ip} ({username}) : {event_type} - {details}")
    try:
...
def get_completions(current_input, current_dir, username, fs):
    base_cmds = [
        ...
        "ftp", "smb", "db", "netlog", "vim", "nano", "iptables", "chown", "chmod"
    ]
    if " " not in current_input:
        ...
        completions = [path for path in fs.keys() if path.startswith(resolved_path)]
        completions += [partial + "/" + content for content in fs.get(current_dir, {}).get("contents", []) if content.startswith(partial.split("/")[-1])]
...
def generate_weekly_report():
    ...
    pdf.cell(0, 10, "Rapport Hebdomadaire du Serveur SSH", ln=True, align="C")
    pdf.ln(10)
    ...
    subject = "Rapport Hebdomadaire Serveur SSH"
    body = "Veuillez trouver en pièce jointe le rapport hebdomadaire des événements du serveur SSH."
    msg = MIMEText(body)
    msg["From"] = SMTP_USER
    msg["To"] = ALERT_TO
    msg["Subject"] = subject
...
def read_line_advanced(chan, prompt, history, current_dir, username, fs, session_log, session_id, client_ip):
    ...
    while True:
        ...
        if char == "\x03":
            chan.send(b"^C\r\n")
            trigger_alert(session_id, "Interrupt", "Ctrl+C pressed", client_ip, username)
            return ""
        if char in ("\x7f", "\x08"):
            if cursor_pos > 0:
                cursor_pos -= 1
...
# ================================
# Traitement des commandes avec alertes détaillées
# ================================
def process_command(cmd, current_dir, username, fs, client_ip, session_id, session_log):
    ...
    # Log de la commande
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    session_log.write(f"{timestamp} - Command executed: {cmd}\n")
    try:
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (timestamp, client_ip, username, cmd, session_id))
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"[!] Erreur lors de l'enregistrement de la commande en DB: {e}")
    ...
    if cmd_name == "cd":
        ...
        if target in ["", "~"]:
            ...
            if target_path == "/root" and username != "root":
                ...
                trigger_alert(session_id, "Access Denied", f"Attempted to access {target_path}", client_ip, username)
            elif target_path in fs and fs[target_path]["type"] == "dir":
                new_dir = target_path
                trigger_alert(session_id, "Directory Change", f"Changed to directory {target_path}", client_ip, username)
            else:
                output = f"bash: cd: {target}: No such file or directory"
                trigger_alert(session_id, "Invalid Directory", f"Attempted to access non-existent directory {target_path}", client_ip, username)
        ...
        if arg_str and not arg_str.startswith("-"):
            ...
            if target_path == "/root" and username != "root":
                ...
                trigger_alert(session_id, "Access Denied", f"Attempted to list {target_path}", client_ip, username)
            elif target_path not in fs or fs[target_path]["type"] != "dir":
                output = f"ls: cannot access '{arg_str}': No such file or directory"
                trigger_alert(session_id, "Invalid Directory", f"Attempted to list non-existent directory {target_path}", client_ip, username)
        if not output:
            contents = fs[target_path]["contents"] if target_path in fs and fs[target_path]["type"]=="dir" else []
            output = "\r\n".join(contents)
            trigger_alert(session_id, "Directory Listing", f"Listed contents of {target_path}", client_ip, username)
        ...
        output = username
        trigger_alert(session_id, "Command Executed", "Checked current user", client_ip, username)
        ...
        trigger_alert(session_id, "Command Executed", "Checked user ID", client_ip, username)

    elif cmd_name == "uname":
        output = "Linux debian 4.19.0-18-amd64 #1 SMP Debian 4.19.208-1 (Debian)" if arg_str else "Linux"
        trigger_alert(session_id, "Command Executed", "Checked system information", client_ip, username)

    elif cmd_name == "echo":
        output = arg_str
        trigger_alert(session_id, "Command Executed", f"Echoed: {arg_str}", client_ip, username)
        ...
        if not arg_str:
            ...
            if file_path in ["/proc/cpuinfo", "/proc/meminfo", "/proc/self/stat", "/proc/mounts", "/var/log/messages", "/var/log/dmesg", "/etc/myconfig.conf"]:
                output = fs[file_path]["content"]()
                trigger_alert(session_id, "File Access", f"Read file {file_path}", client_ip, username)
            elif file_path == "/dev/null":
                output = get_dev_null()
                trigger_alert(session_id, "File Access", "Read /dev/null", client_ip, username)
            elif file_path == "/dev/zero":
                output = get_dev_zero()[:1024]
                trigger_alert(session_id, "File Access", "Read /dev/zero", client_ip, username)
            elif file_path == "/etc/shadow" and username != "root":
                output = f"cat: {arg_str}: Permission denied"
                trigger_alert(session_id, "Access Denied", f"Attempted to read {file_path}", client_ip, username)
            elif file_path in fs:
                node = fs[file_path]
                if node["type"] == "file":
                    output = node["content"]() if callable(node["content"]) else node["content"]
                    trigger_alert(session_id, "File Access", f"Read file {file_path}", client_ip, username)
                    if file_path in HONEY_TOKEN_FILES:
                        trigger_alert(session_id, "Honeytoken Accessed", f"Sensitive file accessed: {file_path}", client_ip, username)
                else:
                    output = f"cat: {arg_str}: Is a directory"
                    trigger_alert(session_id, "Invalid Operation", f"Attempted to read directory {file_path} as file", client_ip, username)
            else:
                output = f"cat: {arg_str}: No such file or directory"
                trigger_alert(session_id, "Invalid File", f"Attempted to read non-existent file {file_path}", client_ip, username)

    elif cmd_name == "rm":
        if not arg_str:
            output = "rm: missing operand"
            trigger_alert(session_id, "Command Error", "rm executed without operand", client_ip, username)
        else:
            target_path = resolve_path(arg_str)
            if target_path in ["/etc/passwd", "/etc/shadow", "/root/.ssh/authorized_keys", "/root/rootkit_detector.sh"]:
                ...
                trigger_alert(session_id, "Critical File Access", f"Attempted to delete {target_path}", client_ip, username)
                ...
                if node["type"] == "file":
                    ...
                    if parent_dir=="/root" and username!="root":
                        ...
                        trigger_alert(session_id, "Access Denied", f"Attempted to delete {target_path}", client_ip, username)
                    else:
                        ...
                        if parent_dir in fs and "contents" in fs[parent_dir]:
                            try:
                                ...
                                pass
                        output = ""
                        trigger_alert(session_id, "File Deletion", f"Deleted file {target_path}", client_ip, username)
                else:
                    output = f"rm: cannot remove '{arg_str}': Is a directory"
                    trigger_alert(session_id, "Invalid Operation", f"Attempted to delete directory {target_path}", client_ip, username)
            else:
                output = f"rm: cannot remove '{arg_str}': No such file or directory"
                trigger_alert(session_id, "Invalid File", f"Attempted to delete non-existent file {target_path}", client_ip, username)
        ...
        trigger_alert(session_id, "Command Executed", "Listed network connections", client_ip, username)

    elif cmd_name == "uptime":
        output = get_dynamic_uptime()
        trigger_alert(session_id, "Command Executed", "Checked system uptime", client_ip, username)

    elif cmd_name == "df":
        output = get_dynamic_df()
        trigger_alert(session_id, "Command Executed", "Checked disk usage", client_ip, username)

    elif cmd_name == "find":
        args = arg_str.split()
        if not args:
            output = "find: missing argument"
            trigger_alert(session_id, "Command Error", "find executed without arguments", client_ip, username)
        else:
            directory = args[0]
            pattern = args[1] if len(args) > 1 else ""
            results = [path for path in fs.keys() if path.startswith(directory) and (pattern=="" or pattern in path)]
            output = "\r\n".join(results)
            trigger_alert(session_id, "Command Executed", f"Searched for files in {directory} with pattern {pattern}", client_ip, username)

    elif cmd_name == "grep":
        args = arg_str.split()
        if len(args) < 2:
            output = "Usage: grep pattern filename"
            trigger_alert(session_id, "Command Error", "grep executed with insufficient arguments", client_ip, username)
        else:
            pattern = args[0]
            filename = args[1]
            file_path = resolve_path(filename)
            if file_path in fs and fs[file_path]["type"] == "file":
                content = fs[file_path]["content"]() if callable(fs[file_path]["content"]) else fs[file_path]["content"]
                lines = content.splitlines()
                matching = [line for line in lines if pattern in line]
                output = "\r\n".join(matching)
                trigger_alert(session_id, "File Search", f"Searched for pattern '{pattern}' in {file_path}", client_ip, username)
            else:
                output = f"grep: {filename}: No such file or directory"
                trigger_alert(session_id, "Invalid File", f"Attempted to grep non-existent file {file_path}", client_ip, username)

    elif cmd_name == "head":
        args = arg_str.split()
        if not args:
            output = "head: missing operand"
            trigger_alert(session_id, "Command Error", "head executed without operand", client_ip, username)
        else:
            filename = args[-1]
            file_path = resolve_path(filename)
            if file_path in fs and fs[file_path]["type"]=="file":
                ...
                lines = content.splitlines()
                output = "\r\n".join(lines[:10])
                trigger_alert(session_id, "File Access", f"Read first 10 lines of {file_path}", client_ip, username)
            else:
        ...
        if not args:
            ...
            trigger_alert(session_id, "Command Error", "tail executed without operand", client_ip, username)
        else:
            filename = args[-1]
            file_path = resolve_path(filename)
            if file_path in fs and fs[file_path]["type"]=="file":
                ...
                lines = content.splitlines()
                output = "\r\n".join(lines[-10:])
                trigger_alert(session_id, "File Access", f"Read last 10 lines of {file_path}", client_ip, username)
            else:
        ...
        trigger_alert(session_id, "Suspicious Command", "Accessed vulnerable database simulation", client_ip, username)
        ...
        trigger_alert(session_id, "Suspicious Command", "Accessed vulnerable configuration", client_ip, username)
        ...
        trigger_alert(session_id, "Suspicious Command", "Accessed vulnerable web simulation", client_ip, username)
        ...
        trigger_alert(session_id, "Suspicious Command", "Attempted FTP connection simulation", client_ip, username)
        ...
        trigger_alert(session_id, "Suspicious Command", "Attempted SMB connection simulation", client_ip, username)
        ...
        trigger_alert(session_id, "Suspicious Command", "Attempted database connection simulation", client_ip, username)
        ...
        output = "\r\n".join(logs) + "\r\n"
        trigger_alert(session_id, "Command Executed", "Checked network logs", client_ip, username)

    elif cmd_name == "history":
        output = "\r\n".join(load_history(username))
        trigger_alert(session_id, "Command Executed", "Viewed command history", client_ip, username)

    elif cmd_name == "sudo":
        if username == "root":
            if arg_str:
                out, new_dir = process_command(arg_str, current_dir, username, fs, client_ip, session_id, session_log)
                output = out
                new_dir = new_dir
                trigger_alert(session_id, "Sudo Command", f"Executed sudo {arg_str}", client_ip, username)
            else:
                output = ""
        else:
            output = (f"[sudo] password for {username}: \n"
                      "Sorry, try again.\nSorry, try again.\nSorry, try again.\nsudo: 3 incorrect password attempts\n")
            trigger_alert(session_id, "Sudo Attempt", "Failed sudo authentication", client_ip, username)
        ...
        trigger_alert(session_id, "SU Attempt", f"Attempted su command: {cmd_name} {arg_str}", client_ip, username)
        ...
        if "http" in arg_str:
            ...
            trigger_alert(session_id, "Download Attempt", f"Attempted download with {cmd_name}: {arg_str}", client_ip, username)
        else:
            output = f"bash: {cmd_name}: command not found"
            trigger_alert(session_id, "Command Error", f"Invalid {cmd_name} command", client_ip, username)

    elif cmd_name in ["apt-get"]:
        output = "E: Could not open lock file /var/lib/dpkg/lock-frontend - open (13: Permission denied)"
        trigger_alert(session_id, "Command Executed", "Attempted apt-get", client_ip, username)

    elif cmd_name in ["dpkg"]:
        output = "dpkg: error: must be root to perform this command"
        trigger_alert(session_id, "Command Executed", "Attempted dpkg", client_ip, username)

    elif cmd_name in ["make"]:
        output = "make: Nothing to be done for 'all'."
        trigger_alert(session_id, "Command Executed", "Attempted make", client_ip, username)
        ...
        output = f"bash: {cmd_name}: command not found"
        trigger_alert(session_id, "File Transfer Attempt", f"Attempted {cmd_name}: {arg_str}", client_ip, username)

    elif cmd_name in ["chmod", "chown"]:
        output = f"{cmd_name}: Operation not supported in this environment"
        trigger_alert(session_id, "Command Executed", f"Attempted {cmd_name}: {arg_str}", client_ip, username)

    elif cmd_name in ["vim", "nano"]:
        output = f"{cmd_name}: Interactive editor not supported in this environment"
        trigger_alert(session_id, "Command Executed", f"Attempted to use {cmd_name}", client_ip, username)

    elif cmd_name in ["bash", "sh"]:
        output = ""
        trigger_alert(session_id, "Command Executed", f"Started new {cmd_name} shell", client_ip, username)

    elif cmd_name in ["netcat", "nc"]:
        output = f"bash: {cmd_name}: command not found"
        trigger_alert(session_id, "Suspicious Command", f"Attempted to use {cmd_name}", client_ip, username)
...
target     prot opt source               destination
"""
        trigger_alert(session_id, "Command Executed", "Listed iptables rules", client_ip, username)
        ...
        trigger_alert(session_id, "Command Executed", f"Executed {cmd_name}", client_ip, username)

    elif cmd_name in ["exit", "logout"]:
        output = ""
        trigger_alert(session_id, "Session Terminated", f"Session closed with {cmd_name}", client_ip, username)

    else:
        output = f"bash: {cmd_name}: command not found"
        trigger_alert(session_id, "Unknown Command", f"Attempted unknown command: {cmd_name}", client_ip, username)
...
class Server(paramiko.ServerInterface):
    ...
    def check_auth_publickey(self, username, key):
        try:
            ...
            fingerprint = "unknown"
        print(f"[!] Tentative d'authentification par clé publique de {self.client_ip} (user={username}, empreinte={fingerprint})")
        trigger_alert(-1, "Public Key Auth", f"Attempted public key authentication for {username}, fingerprint: {fingerprint}", self.client_ip, username)
    ...
    def check_auth_password(self, username, password):
        ...
        if ENABLE_REDIRECTION:
            try:
                ...
                trigger_alert(-1, "Successful Redirect", f"Valid credentials for {username} on {REAL_SSH_HOST}", self.client_ip, username)
            except Exception:
                self.redirect = False
                redirected_flag = 0
                try:
            ...
            if count >= BRUTE_FORCE_THRESHOLD:
                with _brute_force_lock:
                    if self.client_ip not in _brute_force_alerted:
                        ...
                        trigger_alert(self.session_id, "Brute-force Attempt", f"Multiple login attempts from {self.client_ip}", self.client_ip, username)
            conn.close()
        except Exception as e:
...
def handle_connection(client_socket, client_addr):
    ...
    try:
        ...
        # Copie du système de fichiers de base
        ...
        session_filename = os.path.join(SESSION_LOG_DIR, f"session_{client_ip}_{int(time.time())}_{uuid.uuid4().hex}.log")
        ...
        session_log.write(f"{datetime.now().isoformat()} - Session started for user: {session_user} from {client_ip}\n")
        trigger_alert(server.session_id, "Session Started", f"User {session_user} logged in", client_ip, session_user)

        while True:
            prompt = f"{session_user}@debian:{current_dir}$ "
            command = read_line_advanced(chan, prompt, history, current_dir, session_user, fs, session_log, server.session_id, client_ip)
            if command == "":
                continue
            ...
            output, new_dir = process_command(command, current_dir, session_user, fs, client_ip, server.session_id, session_log)
            current_dir = new_dir
        ...
        trigger_alert(server.session_id, "Connection Error", f"Exception occurred: {str(ex)}", client_ip, server.username or "unknown")
    finally:
        try:
            chan.close()
        except Exception:
...
if __name__ == "__main__":
    ...
    print(f"[*] Serveur SSH en écoute sur le port {PORT}...")
